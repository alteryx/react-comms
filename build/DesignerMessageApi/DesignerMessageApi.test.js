...
var _index = _interopRequireDefault(require("./index.ts"));
...
describe('DesignerMessageApi', () => {
  ...
  it('should instantiate itself with a _model, context, and subscriptions', () => {
    const messageBroker = new _index.default(window.Alteryx);
  ...
  it('should call JsEvent when asked to send a message', () => {
    ...
    const messageBroker = new _index.default(window.Alteryx);
    messageBroker.sendMessage('Update_stuff', {
      data: {
        stuff: 'things'
      }
    });
    expect(spyJsEvent).toHaveBeenCalledWith(messageBroker.context, 'Update_stuff', {
      data: {
        stuff: 'things'
      }
    });
  ...
  it('should use the getter for alteryxAppContext to return the contexts language code', () => {
    const messageBroker = new _index.default(window.Alteryx);
    expect(messageBroker.ayxAppContext).toEqual({
      darkMode: false,
      locale: 'US-EN',
      productTheme: {}
    });
